...
1. **Correctness session** (once, 30 minutes, single player is fine): every verify key on (`HaulCacheVerify`,
   ...
   `HomeSearchVerify`, `TerrainSearchVerify`, `SaveSnapshotVerify`, and from 0.4.28 `ReachabilityVerify` (0.4.28 also had `PathFollowVerify`, gone with PathFollow in 0.4.29);
