...
namespace LateGamePerformance
{
    ...
    //
    // The mod walks the same tiles in the same order, but first looks at eight tiles' flags at once, in every
    // layer, and passes over the eight when none is set. For the others it does exactly what the game's loop
    // does, calling the game's own method for each changed cell. A tile passed over is one where the game's loop
    // would only have read flags that are not set, so the same cells are updated in the same order with the same
    // values. (It reads flags in layers a tile does not have as well; a set flag there only means the eight tiles
    // are looked at one by one, as in the game.)
    //
    ...
    internal static class SoilScans
    {
        ...
        public static string TakeStatsLine()
        {
            ...
            string line = string.Format(CultureInfo.InvariantCulture,
                "SoilScans: {0} moisture and contamination passes in {1:0.0} ms ({2:0.000} ms each); {3:0.0}% of " +
                "{4}-tile groups had nothing changed and were passed over; {5} changed cells updated{6}",
                _passes, ms, _passes > 0 ? ms / _passes : 0, _blocks > 0 ? 100.0 * _blocksSkipped / _blocks : 0,
                Block, _changed, _verify ? $"; verify mismatches {_verifyMismatches}" : "");
            _passes = _blocks = _blocksSkipped = _changed = _stopwatchTicks = 0;
            return line;
        }
        ...
        internal static bool MoisturePrefix(object __instance)
        {
            ...
            try
            {
                ...
                return Run(cells, _boolArray(_moistureFlags(simulator)), cells.MapIndex, cells.Visited);
            }
        ...
        internal static bool ContaminationPrefix(object __instance)
        {
            ...
            try
            {
                ...
                return Run(cells, _boolArray(_contaminationFlags(simulator)), cells.MapIndex, cells.Visited);
            }
            catch (Exception exception)
            {
                return Fail(exception);
        ...
        private static bool Run(Cells cells, bool[] flags, MapIndexService mapIndex, List<int> visited)
        {
