...
- 335 automated checks pass against the installed game's assemblies, including 119 patch targets (0.4.25 preview; 0.4.24 had 293 and 105).
...
Not yet done: the stats lines of one 0.4.23 session have been read and the next round built from them as the 0.4.25 preview ([PLAN.md](PLAN.md)), but no feature has been measured before and after in the same save; the verify settings have not been run inside the game (0.4.25 adds one box that turns them all on for a session); and the tree search has not been compared against the game's own search inside a running game. It is still a young mod, so try it on a copy of a save first. The full list is on the [website](https://timbermods.github.io/LateGamePerformance/#status).
