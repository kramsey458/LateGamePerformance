...
Not yet done: no stats lines from a 0.4.14 or 0.4.23 session have been read yet, so the in-game savings are not measured; the verify settings have not been run inside the game; and the tree search has not been compared against the game's own search inside a running game. It is still a young mod, so try it on a copy of a save first. The full list is on the [website](https://timbermods.github.io/LateGamePerformance/#status).
