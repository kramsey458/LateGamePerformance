...
- 0.4.27 is a preview on top of 0.4.26, not yet played. It makes co-op safer rather than faster: the verify settings only measure, so the game always gets the mod's own result whether a player has them on or not; a route or terrain map that fails to build no longer takes other maps with it, is built again on the main thread, and a feature that does turn itself off now shows a dialog asking every player to restart; an entity switched back on in the middle of a tick is ticked as in the unmodded game; the hauling cache's per-tick reset is its own required hook; and the idle-entity check skips its walk over the game's list while the list is unchanged. `tools/benchmark-timberborn.ps1 -SaveCount N` runs the game's own save benchmark with the mods loaded. 393 automated checks pass, including 121 patch targets. All co-op players must update together.
- 0.4.26 (the latest release) has not been played yet. It is the round built from the recorded 0.4.23 session, reviewed twice for production safety (crashes, conflicts with other mods, multiplayer determinism, save-file identity) with the findings fixed, and tested against the game's own classes; the first colony session is still to come, so try it on a copy of a save first, ideally with **Verify every feature** ticked for a few in-game days.
...
- 345 automated checks pass against the installed game's assemblies, including 120 patch targets.
