...
- Preview 0.4.24 (pre-release, not yet played): removes the memory clean-up right after each save that 0.4.17 added. The recorded 0.4.23 session showed it as a 0.9 to 1.3 s freeze at every save on a 3 GB heap, in place of the 100 to 190 ms collection it was meant to pre-empt. Nothing else changes.
...
- 293 automated checks pass against the installed game's assemblies, including 105 patch targets.
...
Not yet done: the stats lines of one 0.4.23 session have been read (the findings and the next round are in [PLAN.md](PLAN.md)), but no feature has been measured before and after in the same save; the verify settings have not been run inside the game; and the tree search has not been compared against the game's own search inside a running game. It is still a young mod, so try it on a copy of a save first. The full list is on the [website](https://timbermods.github.io/LateGamePerformance/#status).
