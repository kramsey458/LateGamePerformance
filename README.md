...
- Played in a late-game save of about 350 beavers: noticeably fewer lag spikes as 0.4.8, reported as working well as 0.4.14, and as 0.4.23 reported as working amazingly well.
- 0.4.27 (the latest release) has not been played yet. It builds on 0.4.26 (also not played) and makes co-op safer rather than faster: the verify settings only measure, so the game always gets the mod's own result whether a player has them on or not; a route or terrain map that fails to build no longer takes other maps with it, is built again on the main thread, and a feature that does turn itself off now shows a dialog asking every player to restart; an entity switched back on in the middle of a tick is ticked as in the unmodded game; the hauling cache's per-tick reset is its own required hook; and the idle-entity check skips its walk over the game's list while the list is unchanged. `tools/benchmark-timberborn.ps1 -SaveCount N` runs the game's own save benchmark with the mods loaded. 393 automated checks pass, including 121 patch targets. All co-op players must update together.
- 0.4.26 is the round built from the recorded 0.4.23 session, reviewed twice for production safety (crashes, conflicts with other mods, multiplayer determinism, save-file identity) with the findings fixed, and tested against the game's own classes. Neither it nor 0.4.27 has had its first colony session yet, so try 0.4.27 on a copy of a save first, ideally with **Verify every feature** ticked for a few in-game days.
- 0.4.26 implements the plan drawn from the recorded 0.4.23 session ([PLAN.md](PLAN.md)): see the bullet above. It also carries the guard against other mods' patches on the saving code that a review of 0.4.25 asked for (0.4.25 was never played). It includes 0.4.24's removal of the memory clean-up right after each save, which that session showed as a 0.9 to 1.3 s freeze at every save on a 3 GB heap. Two boxes on the settings page measure live memory and write a memory snapshot for the heap question the session left open.
...
Not yet done: 0.4.26 and 0.4.27 have not been played in a colony (0.4.23 was); the stats lines of one 0.4.23 session have been read and the round built from them, but no feature has been measured before and after in the same save; the verify settings have not been run inside the game (since 0.4.26 one box turns them all on for a session, and since 0.4.27 they only measure, so one player may have them on in co-op); and the tree search has not been compared against the game's own search inside a running game. It is still a young mod, so try it on a copy of a save first. The full list is on the [website](https://timbermods.github.io/LateGamePerformance/#status).

## Install

1. Install **Harmony** (2.4.1 or newer) and **Mod Settings** from the Steam Workshop.
2. Close Timberborn and extract the [release ZIP](https://github.com/timbermods/LateGamePerformance/releases/latest) into `Documents\Timberborn\Mods`. It contains one `LateGamePerformance` folder. Previews, when there are any, are on the same page as pre-releases. Previews, when there is one, are on the same page as pre-releases.
