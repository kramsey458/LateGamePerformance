...
- **Three more per-tick chores leave the main thread (preview 0.4.23, not yet played).** The water level of every plant is now read on the water worker right after the water map is copied there, so the tick only applies the changes; the list of soil cells whose moisture or contamination changed is written by the worker that finishes the soil simulation, instead of the main thread scanning the whole map for it; and the every-tick search for a beaver to move into a home asks the same beavers the same questions, in the same order, without the component lookup and the enumerators the game makes for each of them. Each gives the game's own result and has a verify setting.
- **Saves freeze the game for less (preview 0.4.22, not yet played).** The snapshot a save starts with was the last part of a save still on the main thread. Trees, crops, paths, levees and platforms, nearly all of a big colony, are now snapshotted on worker threads, using only components whose saving code has been read; beavers and workplaces stay on the main thread, which does its share alongside. The save is the same file.
- **Trees, crops and paths stop being visited every tick (preview 0.4.21, not yet played).** Every one of them is a tickable entity whose only tick part is switched off nearly all the time, yet the game visits all ten thousand of them on every tick. The mod ticks only entities with a part switched on, in the game's own order, and moves entities in and out the moment a part is switched. Same result, about a third of the tick's entity work gone.
...
- Preview 0.4.23 (pre-release): the plant water levels and the soil changed-cell lists are made on the workers that already hold the data, and the home search drops its per-beaver overhead; these were the last of the proposals from the code review of 0.4.22; not yet played. Preview 0.4.22 (pre-release): the save snapshot of trees, crops, paths, levees and platforms moves to worker threads, with a verify box; the top bar and a selected entity's reachability check refresh less often; and a code review of 0.4.21 and 0.4.22 was done and its findings fixed; not yet played. Preview 0.4.21 (pre-release): entities with nothing to tick are left out of the tick loop; not yet played. 0.4.20 (the latest release) brings together 0.4.15 to 0.4.19, **none of which has been played in a colony yet**; every part is tested against the game's assemblies, and the terrain search against the game's real search classes. What they add: terrain path searches from the same tile continue instead of restarting (0.4.18), aimed at the single slow beaver ticks and the end-of-day slump; after a long frame (an autosave, a memory clean-up) the simulation no longer catches up all at once (0.4.16); the memory clean-up after each save moves into the save frame, the audio listener and two interface systems stop redoing their work every frame, and off-screen animated objects skip their pose update (0.4.17); the district resource counts stay on worker threads with MixedStorage installed (0.4.15); the diagnostics timers and the terrain search check are boxes on the settings page (0.4.19); and 0.4.20 fixes what a code review of all of it found. 0.4.10 to 0.4.14: one checkbox on the settings page; a faster tree search when lumberjack flags are full; plant water checks, terrain route maps and district resource counts on worker threads; autosaves and menu saves finish on a worker thread, so the freeze is roughly halved; the water map copy on a worker thread, faster soil scans and less water rendering work (0.4.14). See [TECHNICAL.md](TECHNICAL.md).
...
- 297 automated checks pass against the installed game's assemblies, including 106 patch targets.
...
Not yet done: 0.4.15 to 0.4.23 have not been played in a colony, no stats lines from a 0.4.14 session have been read yet, and the tree search has not been compared against the game's own search inside a running game. It is still a young mod, so try it on a copy of a save first. The full list is on the [website](https://timbermods.github.io/LateGamePerformance/#status).
