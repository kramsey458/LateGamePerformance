...
- **Fewer and smaller hitches around saves and menus (preview 0.4.17, not yet played).** The memory clean-up that followed every autosave a second later now happens inside the save's own frame; the audio listener is placed only when the camera moved instead of casting rays every frame; and the alert lists and the selected entity's panel refresh every few frames instead of every one. None of it touches the simulation.
...
- Hauling lists and tree searches are the game's own results, and rebuilt route maps have exactly the game's contents.
...
- 243 automated checks pass against the installed game's assemblies, including 83 patch targets.
